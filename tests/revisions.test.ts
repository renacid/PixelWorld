...
import { CUSTOM_LAYOUTS } from '../src/stages/layouts';
...
describe('追加仕様の回帰確認', () => {
  ...
  it('手作り森マップと、水面の通行不可・視線通過を検証', () => { CUSTOM_LAYOUTS[99] = { rows: ['###########', '#.........#', '#..##.....#', '#..##.~~..#', '#.....~~..#', '#.........#', '#.........#', '#.........#', '#.........#', '#.........#', '###########'], legend: { '#': 4, '.': 3, '~': 5 }, spawn: { x: 2, y: 2 }, objects: [], enemies: [], trapPlacements: [] }; try { const { map } = generateMap({ ...STAGES[0], id: 99, width: 11, height: 11 }, new Random(1)); expect(wall(map, { x: 6, y: 3 })).toBe(true); expect(lineOfSight(map, { x: 5, y: 3 }, { x: 8, y: 3 })).toBe(true); expect(lineOfSight(map, { x: 2, y: 2 }, { x: 5, y: 2 })).toBe(false); } finally { delete CUSTOM_LAYOUTS[99]; } });
});
