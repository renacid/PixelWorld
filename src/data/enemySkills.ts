...
export type EnemySkillDefinition = {
  requiresMode?: string; cooldown?: number; diagonalRange?: number; visual: 'stone' | 'strike' | 'healingGlow'; sound: SoundCue; name: string; mpCost: number; chance: number; minRange: number; maxRange: number;
  target: 'player' | 'any'; cardinalOnly: boolean; requiresSight: boolean;
  effect: {type:'modeBuff';mode:string;duration:number} | {type:'randomStrike';radius:number;objects:boolean;attribute:Attribute;damageMin:number;damageMax:number} | {type:'fallingStrike'} | {type:'earthPollen'} | {type:'charge';ratio:number;selfStunTurns?:number} | {type:'elementArmor';attribute:'ice';duration:number;chance:number;ratio:number} | {type:'extraActions';count:number} | {type:'sweep';damageMin:number;damageMax:number;attribute:Attribute} | {type:'dash';steps:number} | (({ type: 'approachStrike'; steps: number } | { type: 'projectile' } | { type: 'melee' }) & { damageMin: number; damageMax: number; attribute: Attribute }) | { type: 'teleport'; radius: number } | { type: 'restoreMp'; amount: number; allowFull?: boolean } | { type: 'allyBuff'; radius: number; duration: number; attackMultiplier: number; detectionBonus: number };
};
export const ENEMY_SKILLS: Record<string, EnemySkillDefinition> = {
  windStrike:{name:'風攻撃',mpCost:2,chance:.2,minRange:1,maxRange:1,target:'any',cardinalOnly:false,requiresSight:false,visual:'strike',sound:'magicCast',effect:{type:'randomStrike',radius:1,objects:false,attribute:'wind',damageMin:1,damageMax:1}},
  strongThunderArmor:{name:'強雷装',mpCost:3,chance:.3,minRange:0,maxRange:0,target:'any',cardinalOnly:false,requiresSight:false,visual:'healingGlow',sound:'magicCast',effect:{type:'modeBuff',mode:'thunderMode',duration:5}},
  thunderBall:{name:'サンダーボール',requiresMode:'thunderMode',mpCost:3,chance:.3,minRange:1,maxRange:4,target:'any',cardinalOnly:true,requiresSight:true,visual:'stone',sound:'magicCast',effect:{type:'projectile',attribute:'thunder',damageMin:1.2,damageMax:1.5}},
  randomBolt:{name:'落雷',requiresMode:'thunderMode',mpCost:3,chance:.3,minRange:0,maxRange:2,target:'any',cardinalOnly:false,requiresSight:false,visual:'strike',sound:'magicCast',effect:{type:'randomStrike',radius:2,objects:true,attribute:'thunder',damageMin:1,damageMax:1.3}},
 fallingStrike:{name:'落下攻撃',mpCost:3,chance:.2,minRange:0,maxRange:2,target:'any',cardinalOnly:false,requiresSight:false,visual:'strike',sound:'rocks',effect:{type:'fallingStrike'}},
 earthPollen:{name:'花粉',mpCost:3,chance:.3,minRange:1,maxRange:3,target:'any',cardinalOnly:false,requiresSight:false,visual:'strike',sound:'magicCast',effect:{type:'earthPollen'}},
  chargingStrike:{name:'突進',mpCost:3,chance:1,minRange:2,maxRange:4,target:'any',cardinalOnly:true,requiresSight:true,visual:'strike',sound:'strike',effect:{type:'charge',ratio:1.2,selfStunTurns:1}},
  iceArmor:{name:'氷装',mpCost:2,chance:.2,minRange:0,maxRange:0,target:'any',cardinalOnly:false,requiresSight:false,visual:'healingGlow',sound:'ice',effect:{type:'elementArmor',attribute:'ice',duration:10,chance:.5,ratio:.3}},
