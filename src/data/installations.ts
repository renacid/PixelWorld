...
export type InstallationKind = 'pot' | 'goblinNest';
/** 確率は0～1。poolのweightは種類間の相対比率。 */
export type InstallationDefinition = { name: string; drop?: { chance: number; pool: Weighted<ItemId>[] }; spawn?: { radius: number; chance: number; max: number; pool: Weighted<EnemyKind>[] } };
export const INSTALLATIONS: Record<InstallationKind, InstallationDefinition> = {
  ...
  goblinNest: { name: 'ゴブリンの巣穴', spawn: { radius:7, chance:.3, max:5, pool:[{value:'goblin',weight:5},{value:'goblinArcher',weight:3},{value:'goblinMage',weight:1},{value:'goblinFighter',weight:1}] } },
};
/** overridesで巣穴ごとにも抽選表・上限を変更可能。spawnedは累計で保存。 */
export type Installation = { id:string; kind:InstallationKind; position:Point; spawned:number; requiredForGoal?:boolean; overrides?:Partial<InstallationDefinition> };
