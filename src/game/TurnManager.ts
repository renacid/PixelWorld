...
export function endTurn(state: SaveData, castId?: SkillId): void {
  for (const id of Object.keys(state.cooldowns) as SkillId[]) if (id !== castId && state.passiveTriggeredAt?.[id] !== state.playerActionCount) state.cooldowns[id] = Math.max(0, (state.cooldowns[id] ?? 0) - 1);
  ...
  state.mapState.fields = state.mapState.fields.filter(f => f.skillKind ? f.remainingTurns > 0 : --f.remainingTurns > 0);
