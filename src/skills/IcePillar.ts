...
export function icePillarCells(s:SaveData,direction:Direction):Point[]{
 ...
 const free=(c:Point)=>!cells.some(t=>same(t,c))&&canStand(s.mapState,{...p,cells:[{x:0,y:0}],id:'pillar-probe'},c,actors)&&!s.mapState.objects.some(o=>same(o.position,c))&&!s.mapState.traps?.some(t=>same(t.position,c))&&!s.mapState.playerTraps?.some(t=>same(t.position,c))&&!s.mapState.fields.some(f=>same(f.position,c));
 const first={x:p.position.x+v.x,y:p.position.y+v.y};if(!free(first))return cells;cells.push(first);
...
export function placeIcePillars(s:SaveData,direction:Direction):Point[]{
 const cells=icePillarCells(s,direction),level=effectiveLevel(s.skillBag,s.skillLevels,'icePillar');
