...
export function moveToward(map: MapState, a: Actor, target: Point, blockers: Actor[]): void {
  ...
  if (chosen) { a.facing = faceToward(a, chosen); a.position = chosen; }
}
export function actEnemy(map: MapState, enemy: Actor, targets: Actor[], blockers: Actor[], rng: Random, attack: (a: Actor, b: Actor) => void, action = 0): void {
  ...
  if (target) {
    ...
    enemy.facing = faceToward(enemy, target.position);
  ...
  if (enemy.mode === 'hostile' && enemy.lastSeen && enemy.pursuitLeft > 0) {
    ...
    if (enemy.pattern === 'patrol' && rng.next() < .35) {
      ...
      if (canStand(map, enemy, p, blockers)) { enemy.facing = faceToward(enemy, p); enemy.position = p; }
    }
  }
}
