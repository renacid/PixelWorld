...
root.innerHTML=`<header><div><small>PIXEL WORLD / DEVELOPER TOOLS</small><h1>ダンジョン工房</h1></div><nav><button id="new">新規</button><button id="load">JSONを開く</button><button id="save">下書き保存</button><button id="json">JSON出力</button><button id="export" class="primary">TypeScript出力</button></nav></header>
